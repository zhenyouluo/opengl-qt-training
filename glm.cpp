...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    for (i = 0; i < nummaterials; i++) {
		...
		model->materials[i].hasTex = false;
    }
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'n':               /* newmtl */
			...
			model->materials[nummaterials].hasTex = false;
            break;
		case 'm':
			fgets(buf, sizeof(buf), file);
			sscanf(buf, "%s %s", buf, buf);
			model->materials[nummaterials].texName = strdup(buf);
			model->materials[nummaterials].hasTex = true;
			break;
        case 'N':
